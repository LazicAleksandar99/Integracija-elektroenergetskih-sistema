...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
			...
			try
			{
				...
				do
				{
                    ...
                    else if (str == "4")
                    {
                        try
                        {
                            tgda.PrintElementById(InputGlobalId());
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("PrintElementById failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
                    else if (str == "5")
                    {
                        try
                        {
                            tgda.PrintAllIdesByModels();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("PrintAllIdesByModels failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
                    else if (str == "6")
                    {
                        try
                        {
                            tgda.GetExtendValuesForAllModels();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetExtendValuesForAllModels failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
                    else if (str == "7")
                    {
                        try
                        {
                            tgda.GetValuesOfBidTimeSeriesConnectedWithHighesReasonId();
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("GetValuesOfBidTimeSeriesConnectedWithHighesReasonId failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
		...
		private static void PrintMenu()
		{
            ...
            Console.WriteLine("\t4) Print values by Id");
            Console.WriteLine("\t5) Print id for all models");
            Console.WriteLine("\t6) Get extent values for all models");
            Console.WriteLine("\t7) Get values of timeSeries connected with highest reason Id");
            Console.WriteLine("\t8) Test apply update");
