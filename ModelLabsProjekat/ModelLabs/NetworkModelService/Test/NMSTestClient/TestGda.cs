	...
	{			
		...
		public List<long> GetExtentValues(ModelCode modelCode)
		{
			...
			{				
				...
				iteratorId = GdaQueryProxy.GetExtentValues(modelCode, properties);//id prvog el
				resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);//koliko jos ima el
        ...
        public void PrintElementById(long globalId)
        {
            string message = "Getting values by id method started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                ...
                string itemName = properties[0].ToString();
                itemName = itemName.Split('_')[0];
                message = $"\t{itemName} ID = {rd.Id}\n";
                for (int i = 0; i < properties.Count; i++)
                {
                    if (rd.Properties[i].Type.ToString() == "ReferenceVector")
                    {
                        message += $"\t{properties[i]} = {rd.Properties[i].PropertyValue.LongValues.Count}\n";
                    }
                    else
                    {
                        message += $"\t{properties[i]} = {rd.Properties[i]}\n";
                    }
                }
            ...
            {
                message = string.Format("Printing elements by id method for entered id = {0} failed.\n\t{1}", globalId, e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
        }

        public void PrintAllIdesByModels()
        {
            string message = "Printing all IDs for models has started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            List<ResourceDescription> rdList = new List<ResourceDescription>();
            List<ModelCode> concreatModels = new List<ModelCode> { ModelCode.REASON, ModelCode.PROCESS,
                ModelCode.MARKETDOCUMENT, ModelCode.BIDTIMESERIES, ModelCode.MEASUREMENTPOINT };
            try
            {
                foreach(var code in concreatModels)
                {
                    List<long> allElements = GetExtentValues(code);

                    Console.WriteLine("\n\tModelCode." + code.ToString());
                ...
                message = "PrintAllIdesByModels successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            catch (Exception e)
            {
                message = string.Format("PrintAllIdesByModels failed.\n\t{1}", e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
        }

        public List<long> GetExtendValuesForAllModels()
        {
            string message = "GetExtendValuesForAllModels started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetExtendValuesForAllModels_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                foreach (var code in concreatModels)
                {
                    List<long> allElements = GetExtentValues(code);
                ...
                message = "GetExtendValuesForAllModels successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            catch (Exception e)
            {
                message = string.Format("GetExtendValuesForAllModels failed.\n\t{1}", e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
        ...
        public List<ResourceDescription> GetValuesOfBidTimeSeriesConnectedWithHighesReasonId()
        {
            string message = "Getting values of bidTimeSeries that are connedted with highest id reason started.";
            Console.WriteLine(message);
            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            ...
            try
            {
                List<long> allReasons = GetExtentValues(ModelCode.REASON);
                long highestReasonId = allReasons.Max();

                List<long> allBidTimeSeriess = GetExtentValues(ModelCode.BIDTIMESERIES);

                for (int i = 0; i < allBidTimeSeriess.Count; i++)
                {
                    rd = GetValues(allBidTimeSeriess[i]);
                    if (rd.Properties[13].PropertyValue.LongValue == highestReasonId)
                        rdList.Add(rd);
                }

                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetValuesOfBidTimeSeriesConnectedWithHighesReasonId_Results.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                if (rdList.Count > 0)
                {
                    for (int i = 0; i < rdList.Count; i++)
                    {
                        rdList[i].ExportToXml(xmlWriter);
                    }
                }
                xmlWriter.Flush();

                foreach (var r in rdList)
                {
                    PrintElementById(r.Id);
                }

                message = "Getting values of bidTimeSeries that are connedted with highest id reason successfully finished.";
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            catch (Exception e)
            {
                message = string.Format("Getting values of bidTimeSeries that are connedted with highest id reason failed.\n\t{1}", e.Message);
                Console.WriteLine(message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
            }
            finally
