...
namespace FTN.Common
{
	...
	public class Property : IComparable
	{
		...
		public Property(ModelCode id, decimal decimalValue)
		{
			this.id = id;
			this.value = new PropertyValue();
			...
			else
			{
				...
				switch (first.Type)
				{
					...
					case PropertyType.DateTimeVector:
						...
						break;
					case PropertyType.DecimalVector:
		...
		public void SetValue(decimal decimalValue)
		{
			PropertyType type = this.Type;
			if (type == PropertyType.Decimal)
			{
				value.FloatValue = (float)decimalValue;
			}
			else
			{
				string errorMessage = String.Format("Failed to set value for property ({0}) because specified value ({1}) is not compatible with property type ({2}).", this.id, decimalValue, type);
				CommonTrace.WriteTrace(CommonTrace.TraceError, errorMessage);
				throw new Exception(errorMessage);
			}
		}
		...
		public void SetValue(List<decimal> decimalValues)
		{
			PropertyType type = this.Type;
			if (type == PropertyType.DecimalVector)
			{
				List<float> floatValues = new List<float>();
				foreach (decimal decimalValue in decimalValues)
				{
					floatValues.Add((float)decimalValue);
				}
		...
		public decimal AsDecimal()
		{
			...
			{
				string errorMessage = String.Format("Failed to return value from property for specified type ({0}) because it is incompatible with actual property type ({1})", PropertyType.Decimal, type);
				CommonTrace.WriteTrace(CommonTrace.TraceError, errorMessage);
				throw new Exception(errorMessage);
			}
		}
		...
		public List<float> AsDecimals()
		{
			...
			{
				string errorMessage = String.Format("Failed to return value from property for specified type ({0}) because it is incompatible with actual property type ({1})", PropertyType.DecimalVector, type);
				CommonTrace.WriteTrace(CommonTrace.TraceError, errorMessage);
				throw new Exception(errorMessage);
			}
		}
